...
def create_target_csvs(lcs_dir, models_dir, max_lc_length=20610):
    ...
    for file in list(pathlib.Path(lcs_dir).glob('*.fits')):
        ...
        lc_df['#time'] = np.concatenate((np.array(lc.time.value), padding_zeros))
        ...
        lc_df['centroid_x'] = np.concatenate((np.array(lc.centroid_col.value), padding_zeros))
        lc_df['centroid_y'] = np.concatenate((np.array(lc.centroid_row.value), padding_zeros))
        lc_df['motion_x'] = np.concatenate((np.array(lc.mom_centr1.value), padding_zeros))
        lc_df['motion_y'] = np.concatenate((np.array(lc.mom_centr2.value), padding_zeros))
        lc_df['bck_flux'] = np.concatenate((np.array(lc.sap_bkg.value), padding_zeros))
