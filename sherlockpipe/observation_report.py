...
class ObservationReport:
    ...
    def create_report(self):
        ...
        proposed candidate parameters are those exposed in Table 1. The observables included in the Table 4 \
        are obtained based on the constraints of Table 2 and the list of observatories from Table 3.</font>' % \
                       self.object_id
        ...
        tabla1_data = [['T0 (TBJD)', 'Period (d)', 'Duration (h)', 'Depth (ppt)'],
        ...
        table1_colwidth = [3.5 * cm, 3 * cm, 2.5 * cm, 3.5 * cm]
        ...
        tabla2_data = [['Observability', 'Days interval', 'Min altitude (º)', 'Min Moon Dist (º)', 'Max Moon Dist (º)'],
                       [self.observable, self.max_days, self.min_altitude, self.min_dist, self.max_dist]]
        table2_colwidth = [2.5 * cm, 2.5 * cm, 2.5 * cm, 3 * cm, 3 * cm]
        table2_number_rows = len(tabla1_data)
        ...
        # Le damos el estilo alternando colores de filas:
        ObservationReport.row_colors(self.df_observatories, tabla3)
        story.append(tabla3)
        ...
        # Generamos la tabla 4 con los observables:
        table4_header_row = [df_manipulated.columns[:, ].values.astype(str).tolist()]
        table4_data = df_manipulated.values.tolist()
        table4_data2 = []
        for row in table4_data:
            ...
            table4_data2.append(manipulated_data)

        final_data = table4_header_row + table4_data2

        # Creates a table with variable number of row and width:
        table4_number_rows = len(final_data)
        table4_rowwidths = []
        for x in range(table4_number_rows):
            if x == 0:  # Table headers
                table4_rowwidths.append(0.2 * inch)
            else:
                table4_rowwidths.append(1.2 * inch)

        # Creates a table with 5 columns, variable width:
        table4_colwidths = [1.1 * inch, 2.3 * inch, 0.7 * inch, 0.7 * inch, 2.2 * inch]
        tabla4 = Table(final_data, table4_colwidths, table4_rowwidths, repeatRows=1)
        tabla4.setStyle(table_style)
        # Le damos el estilo alternando colores de filas:
        ObservationReport.row_colors(final_data, tabla4)
